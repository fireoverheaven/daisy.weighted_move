...
func weighted_move(src string, weightedDests []randutil.Choice) {
  ...
  err = filepath.Walk(START,
    func(current string, info os.FileInfo, err error) error {
      ...
      logger.Debug("Walking", slog_path)

      if info.IsDir() {
        logger.Warn("Directory, skipping", slog_path)
        return nil
      }
      ...
      logger.Debug("Destination chosen", slog_path, slog_path_dest)
      ...
      defer infile.Close()
      logger.Info("Source file opened", slog_path)
      ...
      logger.Info("Destination file created", slog_path, slog_path_dest)
      defer outfile.Close()

      fast_copy(infile, outfile)
      logger.Debug("File copied", slog_path, slog_path_dest)
      ...
      logger.Info("Source file was successfully moved to destination", slog_path, slog_path_dest)
      return nil
  })
  if err != nil {
    logger.Error(
